...
def process_data(df: pd.DataFrame, dataset_name: str, output_dir: str) -> None:
    ...
    X_train.to_csv(
        os.path.join(output_dir, f"{dataset_name}_X_train.csv"), index=False
    )
    X_test.to_csv(
        os.path.join(output_dir, f"{dataset_name}_X_test.csv"), index=False
    )
    y_train.to_csv(
        os.path.join(output_dir, f"{dataset_name}_y_train.csv"), index=False
    )
    y_test.to_csv(
        os.path.join(output_dir, f"{dataset_name}_y_test.csv"), index=False
    )
